...
module.exports = class UserModel{
   ...
   getResetPasswordModel = async (token) => {
      let err;
      try{
          var user = await userDBModel.findOne({where: {passwordResetToken: token}});
      }catch{
         err = "Erro ao acessar página de troca de senha";
      }

      return new Promise((resolve, reject) => {
          user ? resolve(user) : reject(err || "Token inválido");
      })
   }
